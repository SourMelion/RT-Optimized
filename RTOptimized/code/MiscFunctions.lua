...
function util.swap_entity_inventories(entity_a, entity_b, inventory)
	swap_inventories(
		entity_a.get_inventory(inventory),
		entity_b.get_inventory(inventory)
	)
end
...
function SwapToGhost(player)
	...
	NEWHOST.get_requester_point().trash_not_requested =
		OG.get_requester_point().trash_not_requested
	------ undo crafting queue -------
	local TheList = nil
	...
	for i = 1, #OG.get_inventory(defines.inventory.character_armor) do
		NEWHOST.get_inventory(defines.inventory.character_armor)
			.insert(OG.get_inventory(defines.inventory.character_armor)[i])
	end
	...
	if
		...
		and remote.interfaces["space-exploration"].on_character_swapped
	then
		remote.call(
			"space-exploration",
			"on_character_swapped",
			{ new_character = NEWHOST, old_character = OG }
		)
	end
	...
	if FlyingItem then
		...
		if FlyingItem.player.character then
			...
			for i = 1, #OG2.get_inventory(defines.inventory.character_armor) do
				FlyingItem.player.character
					.get_inventory(defines.inventory.character_armor)
					.insert(OG2.get_inventory(defines.inventory.character_armor)[i])
			end
			...
			FlyingItem.SwapBack.get_requester_point().enabled =
				OG2.get_requester_point().enabled
			FlyingItem.SwapBack.get_requester_point().trash_not_requested =
				OG2.get_requester_point().trash_not_requested
			if
				remote.interfaces["space-exploration"]
				and remote.interfaces["space-exploration"].on_character_swapped
			then
				remote.call(
					"space-exploration",
					"on_character_swapped",
					{ new_character = FlyingItem.SwapBack, old_character = OG2 }
				)
			end
			OG2.destroy()
		else
	...
	elseif player.character then
		...
		then
			remote.call(
				"space-exploration",
				"on_character_swapped",
				{ new_character = PlayerProperties.SwapBack, old_character = OG2 }
			)
		end
		OG2.destroy()
...
function GetOnZipline(player, PlayerProperties, pole)
	...
	local EquippedTrolley = player.character.get_inventory(
		defines.inventory.character_guns
	)[player.character.selected_gun_index].name
	local SpookySlideGhost = pole.surface.create_entity({
		name = "RTPropCar",
		position = {
			pole.position.x + FromXWireOffset,
			pole.position.y + FromYWireOffset,
		},
		--force = TheGuy.force,
