local function FindPath(finish, start)
	local possibilities = {}
	possibilities[start.unit_number] = {
		entity = start,
		FromStart = 0,
		FromFinish = DistanceBetween(start.position, finish.position),
		difficulty = DistanceBetween(start.position, finish.position),
	}
	local analyzed = {}
	local found = false
	while found == false do
		...
		for i, option in pairs(possibilities) do
			if
				option.difficulty <= current.difficulty
				and option.FromFinish < current.FromFinish
			then
				current = option
				ID = i
			end
		...
		for each, connection in
			pairs(
				current.entity.get_wire_connector(
		...
		do
			...
			if
				neighbor.type ~= "entity-ghost"
				and neighbor.type == "electric-pole"
				and ElectricPoleBlackList[neighbor.name] == nil
			then
				local FromStart = current.FromStart
					+ DistanceBetween(current.entity.position, neighbor.position)
				if
					analyzed[neighbor.unit_number] == nil
					and (#neighbor.get_wire_connector(
						defines.wire_connector_id.pole_copper,
						true
					).real_connections > 1 or neighbor.unit_number == finish.unit_number)
					and (
						possibilities[neighbor.unit_number] == nil
						or possibilities[neighbor.unit_number].FromStart > FromStart
					)
				then
					local difficulty = FromStart
						+ DistanceBetween(neighbor.position, finish.position)
					possibilities[neighbor.unit_number] = {
						entity = neighbor,
						FromStart = FromStart,
						FromFinish = DistanceBetween(neighbor.position, finish.position),
...
ClickableStuff = {
	...
	ZiplineAutoPath = function(event, player)
		...
		if
			storage.ZiplineTerminals[event.element.tags.start]
			and storage.ZiplineTerminals[event.element.tags.finish]
		then
			...
			if
				start.valid
				and finish.valid
				and start.electric_network_id == finish.electric_network_id
			then
	...
	RTRenameTerminal = function(event, player)
		...
		storage.ZiplineTerminals[event.element.parent.parent.tags.ID].name =
			TerminalName
		if
			storage.ZiplineTerminals[event.element.parent.parent.tags.ID].tag.valid
		then
			storage.ZiplineTerminals[event.element.parent.parent.tags.ID].tag.text =
				TerminalName
		end
		event.element.parent.TerminalName.destroy()
		event.element.destroy()
		header.add({ type = "label", name = "TerminalName", caption = TerminalName }).style.font =
			"heading-1"
		header.add({
			type = "sprite-button",
			style = "frame_action_button",
	...
	ZiplineCamera = function(event, player)
		...
		local clicked =
			storage.ZiplineTerminals[event.element.parent.parent.tags.ID].entity
		local a = {}
		...
		for each, terminal in pairs(sorted) do
			...
			if
				entity.valid == true
				and entity.electric_network_id == clicked.electric_network_id
				and entity.unit_number ~= clicked.unit_number
			then
				local TerminalButton = layout.add({
					type = "button",
					name = each,
					caption = terminal.name,
					tags = {
						RTEffect = "ZiplineAutoPath",
						start = script.register_on_object_destroyed(clicked),
						finish = script.register_on_object_destroyed(entity),
					},
				})
				TerminalButton.style.font = "heading-1"
				TerminalButton.style.horizontally_stretchable = true
				local cam = layout.add({
					type = "camera",
					caption = "caption",
