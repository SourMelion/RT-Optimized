...
local function interact(event1) -- has .name = event ID number, .tick = tick number, .player_index, and .input_name = custom input name
	...
	if settings.startup["RTThrowersSetting"].value == true then
		PlayerLauncher = player.surface.find_entity(
			"PlayerLauncher",
			{ math.floor(player.position.x) + 0.5, math.floor(player.position.y) + 0.5 }
		)
	else
		PlayerLauncher = nil
	end
	...
	then
		...
		PlayerProperties.PlayerLauncher.direction =
			storage.OrientationUnitComponents[PlayerLauncher.orientation].name
	...
	if ThingHovering then
		...
		if
			ThingHovering.type == "inserter"
			and string.find(ThingHovering.name, "RTThrower-")
			and ThingHovering.name ~= "RTThrower-PrimerThrower"
			and storage.CatapultList[DestroyNumber].RangeAdjustable == true
		then
			local CurrentRange = math.ceil(
				math.abs(
					ThingHovering.drop_position.x
						- ThingHovering.position.x
						+ ThingHovering.drop_position.y
						- ThingHovering.position.y
				)
			)
			if CurrentRange >= ThingHovering.prototype.inserter_drop_position[2] then
				ThingHovering.drop_position = {
					ThingHovering.drop_position.x
						+ (CurrentRange - 2)
							* storage.OrientationUnitComponents[ThingHovering.orientation].x,
					ThingHovering.drop_position.y
						+ (CurrentRange - 2)
							* storage.OrientationUnitComponents[ThingHovering.orientation].y,
				}
			else
				ThingHovering.drop_position = {
					ThingHovering.drop_position.x
		...
		elseif ThingHovering.name == "RTTrainRamp" then
			...
			local OldRange = storage.MagnetRamps[script.register_on_object_destroyed(
				ThingHovering
			)].range - 3
			ThingHovering.destroy()
			...
			magnetRamps.setRange(RampProperties, OldRange, player)
		...
		elseif
			...
			and ThingHovering.get_wire_connector(
		...
		then
			if
				math.sqrt(
					(player.position.x - ThingHovering.position.x) ^ 2
						+ (player.position.y - ThingHovering.position.y) ^ 2
				) <= 3
			then
				if
		...
		elseif
			...
			and PlayerProperties.state == "default"
			and ThingHovering.type == "electric-pole"
			and string.find(player.character.name, "-jetpack")
		then
			player.print({ "zipline-stuff.range" })
		elseif
			player.character
			...
			and PlayerProperties.state == "default"
			and ThingHovering.type == "electric-pole"
			and #ThingHovering.neighbours == 0
		then
	...
	if
		...
		and player.force.technologies["RTFocusedFlinging"].researched == true
	then
	...
	elseif PlayerProperties.RangeAdjusting == true then
		...
		if
			CurrentRange >= prototypes.entity[thrower.name].inserter_drop_position[2]
		then
			CurrentRange = 1.2
		else
