local magnetRamps = require("__RTOptimized__/code/item/Trains/magnet_ramps")
...
local function click(event)
	...
	if clicked then
		if
			storage.BouncePadList[script.register_on_object_destroyed(clicked)] ~= nil
		then
			storage.BouncePadList[script.register_on_object_destroyed(clicked)].arrow.visible =
				not storage.BouncePadList[script.register_on_object_destroyed(clicked)].arrow.visible

		--| Start setting range of magenet ramp
		elseif
			(
				clicked.name == "RTMagnetTrainRamp"
				or clicked.name == "RTMagnetTrainRampNoSkip"
			) and PlayerProperties.SettingRampRange.SettingRange == false
		then
			local ramp = clicked
			...
			local rektangle = rendering.draw_sprite({
				...
				target = {
					ramp.position.x
						+ storage.OrientationUnitComponents[ramp.orientation + 0.25].x
						- (MaxRange + 1)
							/ 2
							* storage.OrientationUnitComponents[ramp.orientation].x,
					ramp.position.y
						+ storage.OrientationUnitComponents[ramp.orientation + 0.25].y
						- (MaxRange + 1)
							/ 2
							* storage.OrientationUnitComponents[ramp.orientation].y,
				},
				x_scale = MaxRange / 2,
				y_scale = 1,
		...
		elseif PlayerProperties.SettingRampRange.SettingRange == true then
			...
			if TheRamp == nil or TheRamp.valid == false then
				...
				then
					...
					if
						(
							PlayerProperties.SettingRampRange.point == 0
							or PlayerProperties.SettingRampRange.point == 0.5
						)
						and TheRamp ~= nil
						...
						and math.abs(
							TheRail.position.x
								- TheRamp.position.x
								- storage.OrientationUnitComponents[TheRamp.orientation + 0.25].x
						) < 2
					...
					then
						...
						magnetRamps.setRange(RampProperties, range, player, true)
					--|| Horizontal ramps
					elseif
						(
							PlayerProperties.SettingRampRange.point == 0.25
							or PlayerProperties.SettingRampRange.point == 0.75
						)
						and TheRamp ~= nil
						and TheRamp.valid == true
						...
						and math.abs(
							TheRail.position.y
								- TheRamp.position.y
								- storage.OrientationUnitComponents[PlayerProperties.SettingRampRange.Setting.orientation + 0.25].y
						) < 2
						and math.abs(TheRail.position.x - TheRamp.position.x)
							<= PlayerProperties.SettingRampRange.range
					then
					...
					game
						.get_player(event.player_index)
						.print({ "magnet-ramp-stuff.StraightRail" })
					player.play_sound({ path = "utility/cannot_build" })
				end
			end

			if
				PlayerProperties.SettingRampRange.rekt
				and PlayerProperties.SettingRampRange.rekt.valid
			then
