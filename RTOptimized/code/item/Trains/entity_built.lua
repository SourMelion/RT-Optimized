...
local magnetRampsStuff = require("__RTOptimized__/code/item/Trains/magnet_ramps")
...
local function handleMagnetRampBuilt(entity)
	...
	magnetRampsStuff.setRange(storage.MagnetRamps[OnDestroyNumber], nil, player)
end
...
local function handleTrainRampPlacerBuilt(entity, player)
	...
	local ramp = entity.surface.create_entity({
		...
		position = math2d.position.add(
			entity.position,
			constants.PLACER_TO_RAMP_SHIFT_BY_DIRECTION[entity.direction]
		),
		direction = entity.direction,
		force = entity.force,
		raise_built = true,
...
local function on_entity_built(entity, player)
	if
		string.find(entity.name, "^RT")
		and (string.find(entity.name, "TrainRamp") or string.find(
			entity.name,
			"ImpactUnloader"
		))
		and string.find(entity.name, "-placer$")
	then
		handleTrainRampPlacerBuilt(entity)
		return true
	elseif
		entity.name == "RTMagnetTrainRamp"
