...
--
function cube_flying_trains.create_tokens_for_inventory(
	FlyingTrain,
	inventory,
	inv_type
)
	for prototype, _ in pairs(storage.Ultracube.prototypes.irreplaceable) do
		...
		if count > 0 then
			...
			local token = remote.call(
				"Ultracube",
				"create_ownership_token",
				prototype,
				count,
...
function cube_flying_trains.create_token_for_burning(FlyingTrain)
	if
		FlyingTrain.CurrentlyBurning
		and storage.Ultracube.prototypes.irreplaceable[FlyingTrain.CurrentlyBurning.name]
	then -- There is a currently burning item for this FlyingTrain and it is an irreplaceable
		if FlyingTrain.Ultracube == nil then
			FlyingTrain.Ultracube = { tokens = {}, do_hint = false }
		end
...
function cube_flying_trains.position_update(FlyingTrain)
	...
	local velocity = {
		x = position.x - FlyingTrain.Ultracube.prev_pos.x,
		y = position.y - FlyingTrain.Ultracube.prev_pos.y,
	}
	for inv_type, tokens in pairs(FlyingTrain.Ultracube.tokens) do
		for _, token_id in ipairs(tokens) do
			remote.call(
				"Ultracube",
				"update_ownership_token",
				token_id,
...
function cube_flying_trains.release_and_insert(
	FlyingTrain,
	inventory,
	inv_type,
	hint_entity
