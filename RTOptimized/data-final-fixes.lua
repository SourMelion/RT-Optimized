...
function MakePrimedProjectile(ThingData, ProjectileType) -------------------------------------------
	log(
		"--------Creating primed projectile for "
			.. ThingData.type
			.. ": "
			.. ThingData.name
			.. "-----------"
	...
	TheProjectile.name = ThingData.name
		.. "-projectileFromRenaiTransportationPrimed"
	...
	if ThingData.capsule_action then --capsules with thrown actions: grenades, combat robots, poison, slowdown
		if ThingData.capsule_action.attack_parameters.ammo_type.action[1] then
			...
			ProjectileInitialAction =
				data.raw.projectile[ThingData.capsule_action.attack_parameters.ammo_type.action[1].action_delivery.projectile].action
		elseif ThingData.capsule_action.attack_parameters.ammo_type.action then
			log(ThingData.type .. ": " .. ThingData.name .. " has 1 action.")
			ProjectileInitialAction =
				data.raw.projectile[ThingData.capsule_action.attack_parameters.ammo_type.action.action_delivery.projectile].action
		end
	elseif
		ThingData.place_result and data.raw["land-mine"][ThingData.place_result]
	then --landmines
		log(
			ThingData.type
				.. ": "
	...
	elseif ProjectileType == "artillery" then -- artillery
		...
		elseif ThingData.ammo_type.action then
			log(ThingData.type .. ": " .. ThingData.name .. " has 1 action.")
			ProjectileInitialAction =
				data.raw.projectile[ThingData.ammo_type.action.action_delivery.projectile].action
			ProjectileFinalAction =
	...
	if
		ThingData.name == "rocket"
		or ThingData.name == "cannon-shell"
		or ThingData.name == "uranium-cannon-shell"
	then
	...
	if
		TheProjectile.initial_action.type
		or (
			TheProjectile.initial_action[1]
			and TheProjectile.initial_action[1].type
			and TheProjectile.initial_action[1].action_delivery
		)
	then
		data:extend({
			TheProjectile,
		...
		for each, SpriteSet in pairs(SpriteSets) do
			if
				(
					...
					or (
						SpriteSet.flags
						and SpriteSet.flags[1] ~= "mask"
						and SpriteSet.flags[1] ~= "shadow"
					)
				) and SpriteSet.draw_as_shadow == nil
	...
	for ThingID, ThingData in pairs(ThingsTable) do
		if ThingData.stack_size then
			...
			if settings.startup["RTBounceSetting"].value == true then
				if
					ThingData.type == "ammo" -- looking for things like rockets, tank shells, missles, etc
					...
					if ThingData.ammo_type.action[1] then
						for i, action in pairs(ThingData.ammo_type.action) do
							if
								...
								and action.action_delivery.type ~= "stream"
								and action.action_delivery.projectile
							then
								MakePrimedProjectile(ThingData, action.action_delivery.type)
							end
						end
					elseif
						ThingData.ammo_type.action
						and ThingData.ammo_type.action.action_delivery
						and ThingData.ammo_type.action.action_delivery.type ~= "stream"
						and ThingData.ammo_type.action.action_delivery.projectile
					then
						MakePrimedProjectile(
							ThingData,
							ThingData.ammo_type.action.action_delivery.type
						)
					end
				elseif
					(
						...
						and (
							...
							or ( -- old capsule action notation
								ThingData.capsule_action.attack_parameters.ammo_type.action
		...
		if settings.startup["RTZiplineSetting"].value == true then
			if ThingData.type == "electric-pole" then
				...
				elseif
					ThingData.connection_points[1].wire
					and ThingData.connection_points[1].wire.copper
				then
					xavg = 0
					yavg = 0
					...
					Points = {
						xavg / #ThingData.connection_points,
						yavg / #ThingData.connection_points,
					}
				end
				if Points ~= nil then
					data:extend({
		...
		if
			Category == "locomotive"
			or Category == "cargo-wagon"
			or Category == "fluid-wagon"
		then
		...
		if
			Category == "character"
			and (not string.find(ThingID, "RTGhost"))
			and (not string.find(ThingID, "-jetpack"))
		then
