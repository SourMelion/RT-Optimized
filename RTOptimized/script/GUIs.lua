...
function ShowZiplineTerminalGUI(player, clicked)
	...
	local TerminalProperties =
		storage.ZiplineTerminals[script.register_on_object_destroyed(clicked)]
	local frame = player.gui.screen.add({
		type = "frame",
		name = "RTZiplineTerminalGUI",
		direction = "vertical",
		tags = { ID = script.register_on_object_destroyed(clicked) },
	})
	frame.force_auto_center()
	add_titlebar(
		player.gui.screen.RTZiplineTerminalGUI,
		{ "zipline-stuff.select" },
		"stigma"
	)
	if clicked.name == "RTZiplineTerminal" then
		local TerminalHeader = player.gui.screen.RTZiplineTerminalGUI.add({
			type = "table",
			name = "TerminalHeader",
			column_count = 3,
		})
		local ttt = TerminalHeader.add({
			type = "label",
			caption = { "zipline-stuff.terminal" },
		})
		ttt.style.font = "heading-1"
		TerminalHeader.add({
			type = "label",
			name = "TerminalName",
			caption = TerminalProperties.name,
		}).style.font =
			"heading-1"
		TerminalHeader.add({
			type = "sprite-button",
		...
		------------- preview selection
		local PreviewHeader = player.gui.screen.RTZiplineTerminalGUI.add({
			type = "table",
			name = "PreviewHeader",
			column_count = 3,
		})
		PreviewHeader.add({
			...
			name = "cam",
			caption = "Camera",
			tags = { RTEffect = "ZiplineCamera" },
		})
		PreviewHeader.add({
			type = "button",
			name = "map",
			caption = "Minimap",
			tags = { RTEffect = "ZiplineMinimap" },
		})
		PreviewHeader.add({
			type = "button",
			name = "none",
			caption = "None",
			tags = { RTEffect = "ZiplineNone" },
		})
	end

	local scroller = player.gui.screen.RTZiplineTerminalGUI.add({
		type = "scroll-pane",
		name = "scroller",
	})
	...
	local layout =
		scroller.add({ type = "table", name = "layout", column_count = 2 })
	local a = {}
	...
	for each, terminal in pairs(sorted) do
		...
		then
			if
				PlayerProperties.preferences == nil
				or PlayerProperties.preferences.ZiplineTerminalPreview == nil
				or PlayerProperties.preferences.ZiplineTerminalPreview == "camera"
			then
...
function ShowDirectorGUI(player, DirectorPad)
	...
	local DirectorPadDestroyNumber =
		script.register_on_object_destroyed(DirectorPad)
	local frame = player.gui.screen.add({
		type = "frame",
		name = "RTDirectorPadGUI",
		direction = "vertical",
		tags = { ID = DirectorPadDestroyNumber },
	})
	frame.force_auto_center()
	add_titlebar(
		player.gui.screen.RTDirectorPadGUI,
		"Director Bounce Pad Filters",
		"stigma"
	)
	...
	for ligma, balls in pairs(sets) do
		...
		frame.add({
			...
			caption = "Going "
				.. direction
				.. " [virtual-signal=DirectorBouncePlate"
				.. direction
				.. "]",
		})
		local UpRow = player.gui.screen.RTDirectorPadGUI.add({
			type = "table",
			name = direction .. "Row",
			column_count = 10,
		})
		for i = 1, 10 do
			local ItemName
			local setting = DirectorPad.get_or_create_control_behavior()
				.get_section(section)
				.get_slot(i).value
