...
magnetRamps.setRange = function(ramp, range, player, message)
	if range == nil then
		if
			ramp.entity.get_or_create_control_behavior().get_section(1).get_slot(1).min
			and ramp.entity
		...
		ramp.entity.get_or_create_control_behavior().get_section(1).set_slot(
			1,
			{ value = { type = "virtual", name = "signal-R", quality = "normal" }, min = ramp.range }
		)
	end

	if range ~= nil then
		if ramp.rangeID then
			ramp.rangeID.destroy()
		end
		for each, tile in pairs(ramp.tiles) do
			tile.destroy()
		end
		ramp.tiles = {}

		local orientationComponent =
			storage.OrientationUnitComponents[ramp.entity.orientation]
		...
		for i = 1, range do
			...
			local a, b = makeMagRampSection(
				centerPosition,
				ramp.entity.surface,
				ramp.entity.orientation
			)
		...
		if player and message and message == true then
			player.print({
				"magnet-ramp-stuff.set",
				range,
				util.format_number(ramp.power.electric_buffer_size, true),
			})
		end
	end
end
