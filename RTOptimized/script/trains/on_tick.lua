...
local function reEnableSchedule(train, schedule, destinationStation, properties) -- happens for the first carriage
	...
	if
		destinationStation
		and destinationStation.valid
		and destinationStation.trains_limit ~= 4294967295
		and destinationStation.connected_rail
	then -- NoSkip ramp pathing to a station with a train limit
		tempStation = {
		...
		if
			train.path_end_stop
			and train.path_end_stop.valid
			and train.path_end_stop.trains_limit ~= 4294967295
			and train.path_end_stop.connected_rail
		then
			tempStation = {
	...
	if properties.LandedTrain and properties.LandedTrain.valid then
		...
		if schedule and schedule.current then
			...
			if dst and dst.wait_conditions then
				...
				if
					firstWaitCond.condition
					and firstWaitCond.condition.first_signal
					and firstWaitCond.condition.first_signal.name == "RTPropCarItem"
				then
	...
	for PropUnitNumber, properties in pairs(storage.FlyingTrains) do
			...
			--||| Try to reform train
				...
				if NewTrain ~= nil then
					if
						...
						and remote.interfaces.VehicleWagon2.set_wagon_data
						and storage.savedVehicleWagons[properties.WagonUnitNumber]
					then
						remote.call(
							"VehicleWagon2",
							"set_wagon_data",
							NewTrain,
							storage.savedVehicleWagons[properties.WagonUnitNumber]
						)
					end
					...
					elseif AngleChange >= 0.375 then
						...
						NewTrain.die()
						for urmum, lol in
							pairs(GuideCar.surface.find_entities_filtered({ position = GuideCar.position, radius = 7 }))
						do
					...
					if NewTrain.valid then
						...
						elseif NewTrain.type == "locomotive" then
							for _, stock in pairs(NewTrain.train.carriages) do
								if stock.name == "RT-ghostLocomotive" then
									stock.destroy()
								end
							end
						end
						...
						if properties.schedule ~= nil then
							reEnableSchedule(
								NewTrain.train,
								properties.schedule,
								properties.destinationStation,
								properties
							)
						end

						if
							(properties.leader == nil)
							or (properties.follower == nil and properties.length == #NewTrain.train.carriages)
							or (#NewTrain.train.locomotives.front_movers > 0 and NewTrain.train.speed > 0)
							or (#NewTrain.train.locomotives.back_movers > 0 and NewTrain.train.speed < 0)
						then
						...
						if properties.leader == nil then
							if
								(
									properties.ghostLoco ~= nil
									and properties.ghostLoco.valid == true
									and properties.RampOrientation == properties.ghostLoco.orientation
								) or (properties.RampOrientation == properties.orientation)
							then
						...
						if NewTrain.type == "locomotive" then
							...
							if NewTrain.burner then
								...
								if storage.Ultracube then
									...
									if properties.Ultracube then -- Ultracube is active and this locomotive has irreplaceables in it
										if properties.Ultracube.tokens[defines.inventory.fuel] then -- There were irreplaceables in the fuel inventory
											CubeFlyingTrains.release_and_insert(
												properties,
												NewTrain.burner.inventory,
										...
										if properties.Ultracube.tokens[defines.inventory.burnt_result] then -- There were irreplaceables in the fuel inventory
											CubeFlyingTrains.release_and_insert(
												properties,
												NewTrain.burner.burnt_result_inventory,
								...
								for each, stack in pairs(properties.BurntFuelInventory) do
									NewTrain.burner.burnt_result_inventory.insert({
										name = stack.name,
										count = stack.count,
									})
								end
						...
						elseif NewTrain.type == "cargo-wagon" then
							...
							for each, stack in pairs(properties.cargo) do
								NewTrain.get_inventory(defines.inventory.cargo_wagon)
									.insert({ name = stack.name, count = stack.count })
							end
							if
						...
						elseif NewTrain.type == "artillery-wagon" then
							for eacg, stack in pairs(properties.artillery) do
								NewTrain.get_inventory(defines.inventory.artillery_wagon_ammo)
									.insert({ name = stack.name, count = stack.count })
							end
						end
					end
				...
				--|||| Failure
					...
					if
						GuideCar.surface.find_tiles_filtered({
							position = GuideCar.position,
							radius = 1,
							limit = 1,
					...
					if
						remote.interfaces.VehicleWagon2
						and storage.savedVehicleWagons[properties.WagonUnitNumber]
						and remote.interfaces.VehicleWagon2.kill_wagon_data
					then --Vehicle wagon destroy message
						remote.call(
							"VehicleWagon2",
							"kill_wagon_data",
							storage.savedVehicleWagons[properties.WagonUnitNumber]
					...
					for urmum, lol in
						pairs(boom.surface.find_entities_filtered({ position = boom.position, radius = 7 }))
					do
						...
						for each, stack in pairs(properties.cargo) do
							...
							if prototypes.entity[ItemName .. "-projectileFromRenaiTransportationPrimed"] then
								...
								for i = 1, quantity do
									...
									local yshift = math.random(
										-math.sqrt((CrashSpread ^ 2) - (xshift * 10) ^ 2),
										math.sqrt((CrashSpread ^ 2) - (xshift * 10) ^ 2)
									) / 10
									GuideCar.surface.create_entity({
		...
		elseif game.tick < properties.LandTick then
			...
			elseif properties.altered == nil and height >= 3.25 and height <= 4.5 and VerticalSpeed < 0 then
				-- downward arc
				local gravity = 1 / 250
				local LandingRunwayDistance = math.abs(GuideCar.speed)
					* (VerticalSpeed + math.sqrt((VerticalSpeed ^ 2) - 2 * gravity * (3 - height)))
					/ gravity -- "Landing strip" length needed for the touchdown animation
				local XLandOffset = LandingRunwayDistance
					* storage.OrientationUnitComponents[GuideCar.orientation].x
					* (GuideCar.speed / math.abs(GuideCar.speed))
				local YLandOffset = LandingRunwayDistance
					* storage.OrientationUnitComponents[GuideCar.orientation].y
					* (GuideCar.speed / math.abs(GuideCar.speed))
				local rails = GuideCar.surface.find_entities_filtered({
		...
		elseif game.tick > properties.LandTick and properties.LandedTrain and properties.LandedTrain.valid then
			...
			if
				properties.follower
				and properties.followerID
				and storage.FlyingTrains[properties.followerID]
				and storage.FlyingTrains[properties.followerID].LandedTrain == nil
			then
			...
			elseif
				#properties.LandedTrain.train.carriages == properties.length or game.tick > properties.LandTick + 300
			then
				--game.print("all here")
				finalizeLandedTrain(PropUnitNumber, properties)
			end
