local trainHandler =
	require("__RenaiTransportation__/script/trains/entity_built")
...
local function entity_built(event)
	...
	if event.player_index then
		...
		if
			...
			and player
				.get_main_inventory()
				.find_item_stack(entity.ghost_prototype.name .. "-Item")
		then
			player
				.get_main_inventory()
				.remove({ name = entity.ghost_prototype.name .. "-Item", count = 1 })
	...
	if entity.type == "inserter" and string.find(entity.name, "RTThrower-") then
		...
		storage.CatapultList[OnDestroyNumber] = {
			entity = entity,
			targets = {},
		...
		if
			string.find(entity.name, "RTThrower-")
			and entity.name ~= "RTThrower-PrimerThrower"
			and entity.force.technologies["RTFocusedFlinging"].researched == true
		then
			properties.RangeAdjustable = true
		end

		if
			string.find(entity.surface.name, " Orbit")
			or string.find(entity.surface.name, " Field")
			or string.find(entity.surface.name, " Belt")
		then
		...
		if
			entity.burner == nil
			and #entity.fluidbox == 0
			and entity.electric_buffer_size ~= nil
		...
		elseif entity.name == "RTThrower-PrimerThrower" then
			...
			storage.PrimerThrowerLinks[OnDestroyNumber2] =
				{ thrower = entity, ready = false } --, box = box}
		end
	elseif entity.name == "PlayerLauncher" then
		entity.operable = false
		entity.active = false
	elseif
		string.find(entity.name, "BouncePlate")
		and not string.find(entity.name, "Train")
	then
		storage.BouncePadList[script.register_on_object_destroyed(entity)] =
			{ TheEntity = entity }
		local PouncePadProperties =
			storage.BouncePadList[script.register_on_object_destroyed(entity)]
		ShowRange = settings.global["RTShowRange"].value
		if
			entity.name == "DirectedBouncePlate"
			or entity.name == "DirectedBouncePlate5"
			or entity.name == "DirectedBouncePlate15"
		then
			--entity.operable = false
	...
	elseif
		entity.name == "RTTrainRamp"
		or entity.name == "RTTrainRampNoSkip"
		or entity.name == "RTMagnetTrainRamp"
		or entity.name == "RTMagnetTrainRampNoSkip"
	then
	...
	elseif entity.name == "RTZiplineTerminal" then
		local OnDestroyNumber = script.register_on_object_destroyed(entity)
		storage.ZiplineTerminals[OnDestroyNumber] = {
			entity = entity,
