...
local function entity_damaged(event)
	...
	if
		(
			event.entity.name == "RTTrainRamp"
			or event.entity.name == "RTTrainRampNoSkip"
			or event.entity.name == "RTMagnetTrainRamp"
			or event.entity.name == "RTMagnetTrainRampNoSkip"
		)
		and event.cause
		...
		and (math.abs(event.entity.orientation - event.cause.orientation) == 0.5 or math.abs(
			event.entity.orientation - event.cause.orientation
		) == 0)
		and (
			event.entity.orientation == 0
				and event.entity.position.y - event.cause.position.y > 0
			...
			or event.entity.orientation == 0.75
				and event.entity.position.x - event.cause.position.x > 0
		)
	then
		...
		local MagnetRampProperties =
			storage.MagnetRamps[script.register_on_object_destroyed(event.entity)]
		if
			(
				event.entity.name == "RTMagnetTrainRamp"
				or event.entity.name == "RTMagnetTrainRampNoSkip"
			)
			and MagnetRampProperties
			and MagnetRampProperties.range ~= 0
			and MagnetRampProperties.power.energy
					/ MagnetRampProperties.power.electric_buffer_size
				>= 0.95
		then
			FlyingTrainProperties.LandTick = math.ceil(
				game.tick + math.abs(MagnetRampProperties.range / (0.8 * event.cause.speed))
			)
			FlyingTrainProperties.MagnetComp = math.ceil(
				game.tick + 130 * math.abs(event.cause.speed)
			) - FlyingTrainProperties.LandTick
		...
		then
			...
			--game.print("no power")
			FlyingTrainProperties.LandTick =
				math.ceil(game.tick + 130 * math.abs(event.cause.speed))
		else
			FlyingTrainProperties.LandTick =
				math.ceil(game.tick + 130 * math.abs(event.cause.speed)) -- remember to adjust follower calculation too
		end

		FlyingTrainProperties.AirTime = FlyingTrainProperties.LandTick
			- FlyingTrainProperties.LaunchTick
		...
		for number, properties in pairs(storage.FlyingTrains) do -- carriages jumping before the ends land
			if
				properties.LandedTrain ~= nil
				and properties.LandedTrain.valid
				and event.cause.unit_number == properties.LandedTrain.unit_number
			then
		...
		if
			(
				event.entity.name == "RTTrainRamp"
				or event.entity.name == "RTMagnetTrainRamp"
			) and FlyingTrainProperties.schedule ~= nil
		then
			if
				FlyingTrainProperties.schedule.current
				== table_size(FlyingTrainProperties.schedule.records)
			then
				FlyingTrainProperties.schedule.current = 1
			else
				FlyingTrainProperties.schedule.current = FlyingTrainProperties.schedule.current
					+ 1
			end
		elseif
			(
				event.entity.name == "RTTrainRampNoSkip"
				or event.entity.name == "RTMagnetTrainRampNoSkip"
			) and FlyingTrainProperties.schedule ~= nil
		then
			FlyingTrainProperties.destinationStation = event.cause.train.path_end_stop
			FlyingTrainProperties.adjustDestinationLimit =
				event.cause.train.path_end_stop -- manual trains don't have this, it will be nill
			if
				FlyingTrainProperties.adjustDestinationLimit
				and event.cause.train.path_end_stop.trains_limit > 0
				and event.cause.train.path_end_stop.trains_limit < 4294967295
			then -- apparently 4294967295 means train limit is disabled
				-- Artifically reserve the station by decrementing the available blocks
				event.cause.train.path_end_stop.trains_limit = event.cause.train.path_end_stop.trains_limit
					- 1
			end
		end
		...
		for number, properties in pairs(storage.FlyingTrains) do
			if
				properties.follower
				and properties.follower.valid
				and event.cause.unit_number == properties.follower.unit_number
			then
				...
				FlyingTrainProperties.destinationStation =
					storage.FlyingTrains[number].destinationStation
				FlyingTrainProperties.adjustDestinationLimit =
					storage.FlyingTrains[number].adjustDestinationLimit
				if
					(
						event.entity.name == "RTMagnetTrainRamp"
						or event.entity.name == "RTMagnetTrainRampNoSkip"
					...
					and MagnetRampProperties
					and storage.FlyingTrains[number].MagnetComp ~= nil
					and (
						storage.FlyingTrains[number].MakeFX == "yes"
						or storage.FlyingTrains[number].MakeFX == "followerY"
					)
				then
					FlyingTrainProperties.LandTick = math.ceil(
						game.tick
							+ math.abs(
								MagnetRampProperties.range / (0.8 * storage.FlyingTrains[number].speed)
							)
					)
					...
					FlyingTrainProperties.LandTick =
						math.ceil(game.tick + 130 * math.abs(storage.FlyingTrains[number].speed))
		...
		if
			...
			and MagnetRampProperties
			and MagnetRampProperties.range ~= 0
			and FlyingTrainProperties.MakeFX == "yes"
		then
			...
			for each, railtile in pairs(MagnetRampProperties.tiles) do
				rendering.draw_animation({
					...
					x_scale = 0.3,
					y_scale = 0.8,
					time_to_live = FlyingTrainProperties.AirTime
						+ ((5.9 * FlyingTrainProperties.length) / (0.8 * event.cause.speed)),
				})
			end
		elseif
			(
			...
			and MagnetRampProperties
			and MagnetRampProperties.range ~= 0
			and FlyingTrainProperties.MakeFX == "NoEnergy"
		then
			for each, guy in pairs(game.connected_players) do
				guy.add_custom_alert(
					MagnetRampProperties.entity,
					{ type = "item", name = "RTMagnetTrainRampItem" },
					"A Magnet Ramp was used without a full buffer!",
					true
				)
			end
		end
		...
		if event.cause.type == "locomotive" and event.cause.burner then
			if storage.About2Jump[event.cause.unit_number] ~= nil then
				FlyingTrainProperties.CurrentlyBurning =
					storage.About2Jump[event.cause.unit_number].BurningFuel
				storage.About2Jump[event.cause.unit_number] = nil
			else
				FlyingTrainProperties.CurrentlyBurning =
					event.cause.burner.currently_burning
			end
			FlyingTrainProperties.RemainingFuel =
				event.cause.burner.remaining_burning_fuel
			...
			if storage.Ultracube then -- Mod is active
				...
				CubeFlyingTrains.create_tokens_for_inventory(
					FlyingTrain,
					event.cause.burner.inventory,
					defines.inventory.fuel
				)
				CubeFlyingTrains.create_tokens_for_inventory(
					FlyingTrain,
					event.cause.burner.burnt_result_inventory,
			...
			FlyingTrainProperties.FuelInventory =
				event.cause.burner.inventory.get_contents()
			FlyingTrainProperties.BurntFuelInventory =
				event.cause.burner.burnt_result_inventory.get_contents()
		elseif event.cause.type == "cargo-wagon" then
			...
			FlyingTrainProperties.cargo =
				event.cause.get_inventory(defines.inventory.cargo_wagon).get_contents()
			FlyingTrainProperties.bar =
				event.cause.get_inventory(defines.inventory.cargo_wagon).get_bar()
			FlyingTrainProperties.filter = {}
			for i = 1, #event.cause.get_inventory(defines.inventory.cargo_wagon) do
				FlyingTrainProperties.filter[i] =
					event.cause.get_inventory(defines.inventory.cargo_wagon).get_filter(i)
			end
			if
				remote.interfaces.ArmoredTrains
				and remote.interfaces.ArmoredTrains.SendTurretList
			then
				...
				if list ~= nil then
					...
					if turret ~= nil then
						FlyingTrainProperties.ammo =
							turret.get_inventory(defines.inventory.turret_ammo).get_contents()
					end
				end
			end
		...
		elseif event.cause.type == "artillery-wagon" then
			FlyingTrainProperties.artillery = event.cause
				.get_inventory(defines.inventory.artillery_wagon_ammo)
				.get_contents()
		end
		...
		if FlyingTrainProperties.leader == nil then
			for each, carriage in pairs(event.cause.train.carriages) do
				if carriage.burner then
					...
					storage.About2Jump[carriage.unit_number].BurningFuel =
						carriage.burner.currently_burning
			...
			for j = 0, event.cause.grid.height - 1 do
				for i = 0, event.cause.grid.width - 1 do
					if event.cause.grid.get({ i, j }) then
						table.insert(
							FlyingTrainProperties.gridd,
							{ xpos = i, ypos = j, EquipName = event.cause.grid.get({ i, j }).name }
						)
					end
				end
			end
		...
		if
			remote.interfaces.VehicleWagon2
			and remote.interfaces.VehicleWagon2.get_wagon_data
		then
			storage.savedVehicleWagons[event.cause.unit_number] =
				remote.call("VehicleWagon2", "get_wagon_data", event.cause) -- returns nil if not a vehicle wagon
			FlyingTrainProperties.WagonUnitNumber = event.cause.unit_number
			script.raise_event(
				defines.events.script_raised_destroy,
				{ entity = event.cause, cloned = true }
			)
	...
	elseif
		event.entity.name == "RTImpactUnloader"
		and event.cause
		and (
	...
	then
		if
			event.cause.train
			and event.cause.train.cargo_wagons
			and math.abs(event.cause.train.speed) > 0.125
		then
			...
			for each, wagon in pairs(event.cause.train.cargo_wagons) do
				if wagon.name == "RTImpactWagon" then
					...
					for eachh, stack in
						pairs(wagon.get_inventory(defines.inventory.cargo_wagon).get_contents())
					do
						...
						if LaunchedAmount > 0 then
							local GroupSize = math.ceil(
								(LaunchedAmount * wagons) / settings.global["RTImpactGrouping"].value
							)
							for _ = 1, math.floor(LaunchedAmount / GroupSize) do
								...
								local xUnit = (
									math.acos(math.cos(2 * math.pi * (wagon.orientation + 0.25)))
									/ (0.5 * math.pi)
								) - 1
								local yUnit = (
									math.acos(math.cos(2 * math.pi * wagon.orientation)) / (0.5 * math.pi)
								) - 1
								local ForwardSpread = math.random(100, 400) * 0.1
								local HorizontalSpread = math.random(-40, 40) * ForwardSpread * 0.01
								local x = wagon.position.x
									+ (ForwardSpread * wagon.speed * xUnit)
									+ (HorizontalSpread * wagon.speed * yUnit)
								local y = wagon.position.y
									+ (ForwardSpread * wagon.speed * yUnit)
									+ (HorizontalSpread * wagon.speed * xUnit)
								local distance =
									math.sqrt((x - wagon.position.x) ^ 2 + (y - wagon.position.y) ^ 2)
								local speed = math.abs(wagon.speed)
									* (distance / (35 * math.abs(wagon.speed)))
									* math.random(45, 100)
									* 0.01
								local arc = -(0.3236 * distance ^ -0.404) -- lower number is higher arc
								local space = false
								if
									string.find(wagon.surface.name, " Orbit")
									or string.find(wagon.surface.name, " Field")
									or string.find(wagon.surface.name, " Belt")
								then
									arc = -99999999999999
								...
								if
									wagon
										.get_inventory(defines.inventory.cargo_wagon)
										.find_item_stack({ name = ItemName, quality = ItemQuality }).item_number
									~= nil
								then --quality crashes
									local CloudStorage = game.create_inventory(1)
									local item, index = wagon
										.get_inventory(defines.inventory.cargo_wagon)
										.find_item_stack({ name = ItemName, quality = ItemQuality })
								...
								if
									storage.Ultracube
									and storage.Ultracube.prototypes.irreplaceable[ItemName]
								then -- Ultracube mod is active, and item is an irreplaceable
							...
							wagon
								.get_inventory(defines.inventory.cargo_wagon)
								.remove({ name = ItemName, count = LaunchedAmount, quality = ItemQuality })
						end
					end
				end
			end
			if event.cause.train.schedule and event.cause.train.manual_mode == false then
				...
				if
					event.cause.train.schedule.current
					== table_size(event.cause.train.schedule.records)
				then
					stor.current = 1
				else
