...
local function on_int()
	...
	for PlayerID, PlayerLuaData in pairs(game.players) do
		if storage.AllPlayers[PlayerID] == nil then
			storage.AllPlayers[PlayerID] = {
				state = "default",
				PlayerLauncher = {},
				zipline = {},
				RangeAdjusting = false,
				SettingRampRange = { SettingRange = false },
				GUI = {},
				preferences = {},
			}
		end
	...
	for ItemName, info in pairs(prototypes.item) do
		if
			info.fuel_top_speed_multiplier
			and info.fuel_top_speed_multiplier
				> prototypes.item[storage.FastestFuel].fuel_top_speed_multiplier
		then
