local function rotate(event)
	...
	if
		(
			event.entity.name == "DirectedBouncePlate"
			or event.entity.name == "DirectedBouncePlate5"
			or event.entity.name == "DirectedBouncePlate15"
		) and storage.BouncePadList[EntityDestroyNumber] ~= nil
	then
	...
	elseif
		(
			event.entity.name == "RTThrower-EjectorHatchRT"
			or event.entity.name == "RTThrower-FilterEjectorHatchRT"
		) and storage.CatapultList[EntityDestroyNumber] ~= nil
	then
		storage.CatapultList[EntityDestroyNumber].sprite.animation_offset =
			storage.EjectorPointing[event.entity.direction]
	end
	...
	if storage.ThrowerPaths[EntityDestroyNumber] ~= nil then -- if the rotated thing is part of a throw path
		for ThrowerUN, TrackedItems in
			pairs(storage.ThrowerPaths[EntityDestroyNumber])
		do -- go through all the throwers/item pairs this thing was a part of
	...
	if event.entity.name == "RTThrower-PrimerThrower" then
		local PrimerThrowerProperties =
			storage.CatapultList[script.register_on_object_destroyed(event.entity)]
		...
		storage.PrimerThrowerLinks[PrimerThrowerProperties.entangled.detector.unit_number] =
			nil
		...
		storage.PrimerThrowerLinks[OnDestroyNumber] =
			{ thrower = event.entity, ready = false } --, box = box}
	end
end
