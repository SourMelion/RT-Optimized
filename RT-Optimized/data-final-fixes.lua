	...
	if (TheProjectile.initial_action.type or (TheProjectile.initial_action[1] and TheProjectile.initial_action[1].type and TheProjectile.initial_action[1].action_delivery)) then
		data:extend({
			TheProjectile,
			{
				...
				selection_box = nil,
				rotation_speed = 1,
				folded_animation = {direction_count=4, filename = "__RT-Optimized__/graphics/nothing.png", size=1},
				graphics_set = {},
...
TheItem = table.deepcopy(data.raw.item[ThingData.minable.result])
	...
	if (TheItem.icon) then
		...
		table.insert(TheItem.icons, {icon = "__RT-Optimized__/graphics/ThrowerInserter/overlay.png",	icon_size = 64, icon_mipmaps = 4})
	end
