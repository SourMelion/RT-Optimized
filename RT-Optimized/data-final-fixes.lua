function MakeProjectile(ThingData, speed)
    ...
    local TheProjectile = {
        ...
        particle = {
            layers = {
                {
                    filename = "__RT-Optimized__/graphics/icon.png",
                    line_length = 1,
                    frame_count = 1,
                    priority = "high",
                    size = 32,
...
function MakePrimedProjectile(ThingData, ProjectileType) -------------------------------------------
    ...
    if (ThingData.capsule_action) then --capsules with thrown actions: grenades, combat robots, poison, slowdown
        if (ThingData.capsule_action.attack_parameters.ammo_type.action[1]) then
            ...
            ProjectileInitialAction =
                data.raw.projectile[
                ThingData.capsule_action.attack_parameters.ammo_type.action[1].action_delivery.projectile
            ].action
        elseif (ThingData.capsule_action.attack_parameters.ammo_type.action) then
            log(ThingData.type .. ": " .. ThingData.name .. " has 1 action.")
            ProjectileInitialAction =
                data.raw.projectile[
                ThingData.capsule_action.attack_parameters.ammo_type.action.action_delivery.projectile
            ].action
        end
    ...
    elseif (ProjectileType == "artillery") then -- artillery
        if (ThingData.ammo_type.action[1]) then
            ...
            for i, action in pairs(ThingData.ammo_type.action) do
                if (action.action_delivery.projectile) then
                    ProjectileInitialAction =
                        data.raw["artillery-projectile"][ThingData.ammo_type.action[1].action_delivery.projectile].action
                    ProjectileFinalAction =
                        data.raw["artillery-projectile"][ThingData.ammo_type.action[1].action_delivery.projectile].final_action
                end
            end
        elseif (ThingData.ammo_type.action) then
            log(ThingData.type .. ": " .. ThingData.name .. " has 1 action.")
            ProjectileInitialAction =
                data.raw["artillery-projectile"][ThingData.ammo_type.action.action_delivery.projectile].action
            ProjectileFinalAction =
                data.raw["artillery-projectile"][ThingData.ammo_type.action.action_delivery.projectile].final_action
        end
        ...
        if (ThingData.ammo_type.action[1]) then
            ...
            for i, action in pairs(ThingData.ammo_type.action) do
                if (action.action_delivery.projectile) then
                    ProjectileInitialAction =
                        data.raw.projectile[ThingData.ammo_type.action[i].action_delivery.projectile].action
                    ProjectileFinalAction =
                        data.raw.projectile[ThingData.ammo_type.action[i].action_delivery.projectile].final_action
                end
            end
        elseif (ThingData.ammo_type.action) then
            log(ThingData.type .. ": " .. ThingData.name .. " has 1 action.")
            ...
            ProjectileFinalAction =
                data.raw.projectile[ThingData.ammo_type.action.action_delivery.projectile].final_action
        end
    end
    ...
    if (ThingData.name == "rocket" or ThingData.name == "cannon-shell" or ThingData.name == "uranium-cannon-shell") then
    ...
    if
        (TheProjectile.initial_action.type or
            (TheProjectile.initial_action[1] and TheProjectile.initial_action[1].type and
                TheProjectile.initial_action[1].action_delivery))
     then
        data:extend(
            {
                TheProjectile,
                {
                    ...
                    folded_animation = {
                        direction_count = 4,
                        filename = "__RT-Optimized__/graphics/nothing.png",
                        size = 1
                    },
                    graphics_set = {},
...
function MakeThrowerVariant(ThingData)
    ...
    if (TheItem.icon) then
        TheItem.icons = {
            ...
            {
                icon = "__RT-Optimized__/graphics/ThrowerInserter/overlay.png",
                icon_size = 64,
                icon_mipmaps = 4
            }
        }
        ...
        table.insert(
            TheItem.icons,
            {icon = "__RT-Optimized__/graphics/ThrowerInserter/overlay.png", icon_size = 64, icon_mipmaps = 4}
        )
    end
    ...
    if settings.startup["RTThrowersDynamicRange"].value == true then
        ...
        ItsRange =
            math.floor(math.sqrt(original_inserter.insert_position[1] ^ 2 + original_inserter.insert_position[2] ^ 2)) *
            10 +
            5
    ...
    TheThrower.hand_base_picture = {
        filename = "__RT-Optimized__/graphics/ThrowerInserter/hr-inserter-hand-base.png",
        priority = "extra-high",
        width = 32,
        height = 136,
        scale = 0.25
    ...
    TheThrower.hand_closed_picture = {
        filename = "__RT-Optimized__/graphics/ThrowerInserter/hr-inserter-hand-closed.png",
        priority = "extra-high",
        width = 72,
        height = 164,
        scale = 0.25
    ...
    TheThrower.hand_open_picture = {
        filename = "__RT-Optimized__/graphics/ThrowerInserter/hr-inserter-hand-open.png",
        priority = "extra-high",
        width = 72,
        height = 164,
        scale = 0.25
...
function MakeCarriageSprites(ThingData)
    ...
    local UpSprites = {
        {
            filename = "__RT-Optimized__/graphics/TrainRamp/trains/base/WheelsVertical.png",
            size = {200, 500},
            scale = 0.5,
            tint = {0.5, 0.5, 0.5}
        }
    ...
    local RightSprites = {
        {
            filename = "__RT-Optimized__/graphics/TrainRamp/trains/base/WheelsHorizontal.png",
            size = {500, 200},
            shift = {0, -0.5},
            scale = 0.5,
            tint = {0.5, 0.5, 0.5}
    ...
    -- local SpriteScale = 1
    local SpriteSets
    if (ThingData.pictures and ThingData.pictures.rotated) then
    ...
    if (SpriteSets ~= nil) then
        for each, SpriteSet in pairs(SpriteSets) do
            if
                ((SpriteSet.flags == nil or
                    (SpriteSet.flags and SpriteSet.flags[1] ~= "mask" and SpriteSet.flags[1] ~= "shadow")) and
                    SpriteSet.draw_as_shadow == nil)
             then -- carriage "body" sprite
                    ...
                    if (SpriteSet.back_equals_front ~= true) then
                        table.insert(
                            UpSprites,
                            {
                                ...
                                y = SpriteSet.height *
                                    math.floor(
                                        (0 % (SpriteSet.line_length * SpriteSet.lines_per_file)) / SpriteSet.line_length
                                    ),
                        ...
                        table.insert(
                            RightSprites,
                            {
                                filename = SpriteSet.filenames[1 + math.floor(0.25 * #SpriteSet.filenames)],
                                x = SpriteSet.width *
                                    ((0.25 * #SpriteSet.filenames * SpriteSet.line_length * SpriteSet.lines_per_file) %
                        ...
                        table.insert(
                            DownSprites,
                            {
                                filename = SpriteSet.filenames[1 + math.floor(0.5 * #SpriteSet.filenames)],
                                x = SpriteSet.width *
                                    ((0.5 * #SpriteSet.filenames * SpriteSet.line_length * SpriteSet.lines_per_file) %
                                        SpriteSet.line_length),
                                y = SpriteSet.height *
                                    math.floor(
                                        ((0.5 * #SpriteSet.filenames * SpriteSet.line_length * SpriteSet.lines_per_file) %
                        ...
                        table.insert(
                            LeftSprites,
                            {
                                filename = SpriteSet.filenames[1 + math.floor(0.75 * #SpriteSet.filenames)],
                                x = SpriteSet.width *
                                    ((0.75 * #SpriteSet.filenames * SpriteSet.line_length * SpriteSet.lines_per_file) %
                                        SpriteSet.line_length),
                                y = SpriteSet.height *
                                    math.floor(
                                        ((0.75 * #SpriteSet.filenames * SpriteSet.line_length * SpriteSet.lines_per_file) %
...
for ThingID, ThingData in pairs(data.raw.inserter) do
    ...
    if (settings.startup["RTThrowersSetting"].value == true and settings.startup["RTModdedThrowers"].value == true) then
        if
            (ThingData.type == "inserter" and ThingData.energy_source.type ~= "void" and
                ThingData.draw_held_item ~= false and
                ThingData.selectable_in_game ~= false and
                ThingData.minable and
                ThingData.minable.result and
                ThingData.rotation_speed ~= 0 and
                ThingData.extension_speed ~= 0 and
                data.raw.item[ThingData.minable.result] ~= nil and
                ThingData.selection_box[1][1] >= -0.5 and
                ThingData.selection_box[1][2] >= -0.5 and
                ThingData.selection_box[2][1] <= 0.5 and
                ThingData.selection_box[2][2] <= 0.5 and
                not string.find(ThingData.name, "RTThrower-"))
         then
    ...
    elseif (settings.startup["RTThrowersSetting"].value == true and settings.startup["RTModdedThrowers"].value == false) then
        if
            (ThingData.name == "burner-inserter" or ThingData.name == "inserter" or ThingData.name == "fast-inserter" or
                ThingData.name == "long-handed-inserter" or
                ThingData.name == "filter-inserter" or
                ThingData.name == "stack-filter-inserter" or
                ThingData.name == "stack-inserter")
         then
            MakeThrowerVariant(ThingData)
        end
    end
    ...
    for ThingID, ThingData in pairs(ThingsTable) do
        if (ThingData.stack_size) then
            ...
            if (settings.startup["RTBounceSetting"].value == true) then
                if
                    (ThingData.type == "ammo" and ThingData.ammo_type.action) -- looking for things like rockets, tank shells, missles, etc
                 then --if this ammo does something
                    if (ThingData.ammo_type.action[1]) then
                        for i, action in pairs(ThingData.ammo_type.action) do
                            if
                                (action.action_delivery and action.action_delivery.type ~= "stream" and
                    ...
                    elseif
                        (ThingData.ammo_type.action and ThingData.ammo_type.action.action_delivery and
                            ThingData.ammo_type.action.action_delivery.type ~= "stream" and
                            ThingData.ammo_type.action.action_delivery.projectile)
                     then
                        MakePrimedProjectile(ThingData, ThingData.ammo_type.action.action_delivery.type)
                    end
                elseif
                    ((Category == "capsule" and --if its a capsule
                        ThingData.capsule_action.type == "throw" and --with a thrown action
                        (-- 0.18.36+ capsule action notation
                        (ThingData.capsule_action.attack_parameters.ammo_type.action and
                            ThingData.capsule_action.attack_parameters.ammo_type.action[1] and
                            ThingData.capsule_action.attack_parameters.ammo_type.action[1].action_delivery and
                            ThingData.capsule_action.attack_parameters.ammo_type.action[1].action_delivery.projectile and
                            data.raw.projectile[
                                ThingData.capsule_action.attack_parameters.ammo_type.action[1].action_delivery.projectile
                            ] and
                             --that has an associated projectile
                            data.raw.projectile[
                                ThingData.capsule_action.attack_parameters.ammo_type.action[1].action_delivery.projectile
                            ].action) or --that does something -- old capsule action notation
                            (ThingData.capsule_action.attack_parameters.ammo_type.action and
                                ThingData.capsule_action.attack_parameters.ammo_type.action.action_delivery and
                                ThingData.capsule_action.attack_parameters.ammo_type.action.action_delivery.projectile and
                                data.raw.projectile[
                                    ThingData.capsule_action.attack_parameters.ammo_type.action.action_delivery.projectile
                                ] and
                                 --that has an associated projectile
                                data.raw.projectile[
                                    ThingData.capsule_action.attack_parameters.ammo_type.action.action_delivery.projectile
                                ].action))) or --that does something
                        data.raw["land-mine"][ThingData.place_result])
                 then
        ...
        if
            (Category == "character" and (not string.find(ThingID, "RTGhost")) and
                (not string.find(ThingID, "-jetpack")))
         then
